...
def luo_sankey(tulot_summa, df_menot_avg, jaama):
    ...
    fig = go.Figure(data=[go.Sankey(node=dict(pad=20, thickness=20, line=dict(color="gray", width=0.5), label=labels, color=node_colors), link=dict(source=sources, target=targets, value=values, color=[node_colors[t].replace('rgb', 'rgba').replace(')', ', 0.3)') for t in targets]))])
        ...
        def process_section(start_idx, end_idx, kategoria):
            ...
            for _, row in section.iterrows():
                ...
                for col_idx in range(2, df.shape[1]):
                    ...
                    if pd.notna(val) and val > 0: data_rows.append({"Kategoria": kategoria, "Selite": selite, "Kuukausi": col_name, "Summa": round(val, 2)})
...
def laske_tulevaisuus(aloitussumma, kk_saasto, korko_pros, vuodet):
    ...
    for kk in range(1, vuodet * 12 + 1):
        ...
        if kk % 12 == 0: data.append({"Vuosi": int(kk / 12), "Oma pääoma": round(oma_paaoma, 0), "Tuotto": round(saldo - oma_paaoma, 0), "Yhteensä": round(saldo, 0)})
    return pd.DataFrame(data)

def analysoi_talous(df_avg, profiili, data_tyyppi, df_raw):
    try:
        kk_lkm = df_raw['Kuukausi'].nunique()
        tulot, menot = df_avg[df_avg['Kategoria']=='Tulo']['Summa'].sum(), df_avg[df_avg['Kategoria']=='Meno']['Summa'].sum()
        jaama = tulot - menot
        sijoitus_keywords = ['sijoitus', 'rahasto', 'osake', 'säästö', 'nordnet', 'op-tuotto', 'ostot', 'etf']
        sijoitukset_summa = sum(row['Summa'] for _, row in df_avg[df_avg['Kategoria']=='Meno'].iterrows() if any(x in str(row['Selite']).lower() for x in sijoitus_keywords))
        todellinen_saasto = jaama + sijoitukset_summa
        kulut_txt = df_avg[df_avg['Kategoria']=='Meno'].nlargest(5, 'Summa').to_string(index=False)
        
        model = genai.GenerativeModel('gemini-2.5-flash')
        prompt = f"""
        ### ROLE: Senior Private Banker
        ### CONTEXT: Analysoit tyypiltään {data_tyyppi} olevaa dataa.
        - Aikaväli: {kk_lkm} kk. Asiakas: {profiili['ika']}v, {profiili['suhde']}, {profiili['lapset']} lasta.
        - Tavoite: {profiili['tavoite']} ({profiili.get('tavoite_summa')}€).
        - Tulot: {tulot:.0f}€/kk, Menot: {menot:.0f}€/kk, Todellinen säästö: {todellinen_saasto:.0f}€/kk.
        - Top Menot: {kulut_txt}
        
        ### TEHTÄVÄ: Luo Markdown-analyysi
        ## 1. Tilannekuva ({data_tyyppi})
        Analysoi {kk_lkm} kk edustavuus ja tavoitteen realistisuus.
        ## 2. Kulurakenteen analyysi
        Skaalaa luvut vuositasolle (12 kk).
    ...
    except Exception as e: return f"Virhe: {str(e)}"
